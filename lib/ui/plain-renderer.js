...
const { formatStatusLine, splitHelpLines, statusIcon } = require('./components/status-list');
const { renderTablePage } = require('./components/table');
...
/**
 * @typedef {{ type: 'task:start'|'task:finish', taskId: string, label?: string, durationMs?: number, success?: boolean, plain?: boolean }} TaskEvent
 */
...
class PlainRenderer {
  ...
  table(rows, columns, options) {
    const rendered = renderTablePage(rows, columns, { showFooter: false, ...(options || {}) });
    if (rendered.lines.length === 0) return;
    console.log(chalk.bold(rendered.lines[0]));
    for (const line of rendered.lines.slice(1)) {
      console.log(line);
    }
    if (rendered.detailLines.length > 0) {
      console.log('');
      for (const line of rendered.detailLines) console.log(line);
    }
  }
  ...
  statusList(groups) {
    ...
    for (const group of groups) {
      ...
      for (const item of group.items) {
        const line = formatStatusLine(index, item).replace(
          item && item.detail ? `[${item.detail}]` : '',
          item && item.detail ? chalk.gray(`[${item.detail}]`) : ''
        );
        ...
        for (const helpLine of splitHelpLines(item && item.help)) {
          console.log(chalk.yellow(`   💡 ${helpLine}`));
        }
