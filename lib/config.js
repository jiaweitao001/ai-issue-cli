 ...
 * @returns {'auto'|number}
 */
function parseVerifyLoopParallelism(value) {
...
const DEFAULT_CONFIG = {
  ...
  verifyLoop: {
    enabled: process.env.AI_ISSUE_VERIFY_LOOP_ENABLED !== 'false',
    maxAttempts: Number(process.env.AI_ISSUE_VERIFY_LOOP_MAX_ATTEMPTS || 3),
    phaseATimeoutSec: Number(process.env.AI_ISSUE_VERIFY_LOOP_PHASE_A_TIMEOUT_SEC || 1800),
    phaseBTimeoutSec: Number(process.env.AI_ISSUE_VERIFY_LOOP_PHASE_B_TIMEOUT_SEC || 2400),
    parallelism: parseVerifyLoopParallelism(process.env.AI_ISSUE_VERIFY_LOOP_PARALLELISM),
    skipGates: process.env.AI_ISSUE_VERIFY_LOOP_SKIP_GATES
      ? process.env.AI_ISSUE_VERIFY_LOOP_SKIP_GATES.split(',').map(s => s.trim()).filter(Boolean)
      : []
  }
};
...
function validateConfig(config) {
  ...
  if (config.verifyLoop !== undefined) {
    const verifyLoop = config.verifyLoop;
    if (!verifyLoop || typeof verifyLoop !== 'object' || Array.isArray(verifyLoop)) {
      errors.push('verifyLoop must be an object');
    } else {
      if (typeof verifyLoop.enabled !== 'boolean') {
        errors.push('verifyLoop.enabled must be a boolean');
      }
      for (const key of ['maxAttempts', 'phaseATimeoutSec', 'phaseBTimeoutSec']) {
        const value = verifyLoop[key];
        if (!Number.isInteger(value) || value <= 0) {
          errors.push(`verifyLoop.${key} must be a positive integer`);
        }
      }
      const parallelism = verifyLoop.parallelism;
      if (parallelism !== 'auto' && (!Number.isInteger(parallelism) || parallelism <= 0)) {
        errors.push('verifyLoop.parallelism must be "auto" or a positive integer');
      }
      if (!Array.isArray(verifyLoop.skipGates) || verifyLoop.skipGates.some(g => typeof g !== 'string' || !g)) {
        errors.push('verifyLoop.skipGates must be an array of non-empty strings');
      }
    }
  }
...
function loadConfig() {
  ...
  let config = {
    ...
    verifyLoop: { ...DEFAULT_CONFIG.verifyLoop }
  };
  if (fs.existsSync(CONFIG_FILE)) {
    try {
      ...
      config = {
        ...
        verifyLoop: userConfig.verifyLoop === undefined
          ? config.verifyLoop
          : (isPlainObject(userConfig.verifyLoop)
              ? { ...(config.verifyLoop || {}), ...userConfig.verifyLoop }
              : userConfig.verifyLoop)
      };
    } catch (e) {
