...
const { readCache: readSiblingPrCache, lookup: lookupSiblingPrCache } = require('./sibling-pr-cache');
 ...
 *
 * Source-of-truth order for each parsed issue ref:
 *   1. BS-07 PR-C cache (sidecar file written by the MCP skill from the
 *      service's /search response). Skips both `resolvePrForIssue` and
 *      `fetchPrDiff` when the entry has pre-trimmed hunks.
 *   2. GitHub REST fallback (existing `resolvePrForIssue` + `fetchPrDiff`
 *      chain) for cache misses — keeps the system working when the
 *      service is unreachable or hasn't been backfilled yet.
 *
 * The cache path is read from `opts.cachePath` (test seam) or the
 * `AI_ISSUE_SIBLING_PR_CACHE_PATH` env var. Cache failures fall through
 * to the GitHub path silently.
 *
 * @param {BuildOptions & { cachePath?: string }} opts
...
async function buildSiblingPrDiffsSection(opts) {
  ...
  const cachePath = opts.cachePath || process.env.AI_ISSUE_SIBLING_PR_CACHE_PATH || '';
  const cache = cachePath ? readSiblingPrCache(cachePath, logger) : {};

  /** @type {Array<{ ref: ParsedIssueRef, prNumber: number, files_changed: string[], hunks: Array<{file: string, hunk: string}> }>} */
  const resolved = [];
  for (const ref of refs) {
    // ── PR-C cache-hit fast path ─────────────────────────────────────
    const cached = lookupSiblingPrCache(cache, ref.repo, ref.issueNumber);
    if (cached) {
      debug(`sibling-pr-diffs: cache hit for ${ref.repo}#${ref.issueNumber} → PR #${cached.pr_number} (${cached.diff_hunks.length} hunks)`);
      resolved.push({
        ref,
        prNumber: cached.pr_number,
        files_changed: Array.isArray(cached.files_changed) ? cached.files_changed : [],
        hunks: cached.diff_hunks
      });
      continue;
    }

    // ── GitHub fallback (cache miss) ─────────────────────────────────
    let prNumber = null;
    try {
