...
function loadManifest(kbDir) {
  ...
  if (!manifest || typeof manifest !== 'object' || Array.isArray(manifest)) {
    throw makeKbError(
      kbErrors.KB_CORRUPTED,
      `Knowledge base manifest must be a JSON object: ${manifestPath}`,
      { path: manifestPath }
    );
  }
...
function verifySha256(filePath, expectedHex) {
  const expectedNormalized = typeof expectedHex === 'string' ? expectedHex.toLowerCase() : expectedHex;
  return new Promise((resolve, reject) => {
    ...
    stream.on('end', () => {
      ...
      if (actual === expectedNormalized) {
        ...
        resolve({ ok: false, actual, expected: expectedNormalized });
      }
    });
  });
}
