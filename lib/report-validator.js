 ...
 *
 * Section/field name lists live in `data/report-sections.json` and are also
 * consumed by `skills/report-validator/index.js` (MCP-side validation) so the
 * two stay in lockstep without duplicated literals.
 */

const SECTIONS = require('../data/report-sections.json');

// Required sections for Phase 1 Research Report
const PHASE1_REQUIRED_SECTIONS = SECTIONS.PHASE1_REQUIRED_SECTIONS;

// Required sections for Phase 2 Solution Report (matched without leading number)
...
const PROBLEM_ANALYSIS_FIELDS = SECTIONS.PROBLEM_ANALYSIS_FIELDS;

// Required checklist items
const CHECKLIST_ITEMS = SECTIONS.CHECKLIST_ITEMS;
...
function validateSolutionReport(content) {
  ...
  // Check required sections (match with any numbering: "## 1. Problem Analysis", "## 2. Problem Analysis", etc.)
  for (const section of PHASE2_REQUIRED_SECTIONS) {
    const pattern = new RegExp(`^## \\d+\\.\\s*${section.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'm');
  ...
  const checklistMatch = content.match(/## \d+\.\s*Pre-submission Checklist([\s\S]*?)## \d+\./);
  const checklistSection = checklistMatch ? checklistMatch[1] : null;
  if (checklistSection) {
