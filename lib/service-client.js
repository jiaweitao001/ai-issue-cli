...
// Header names sent by the server when a subscription rotation is in progress.
// Keep in sync with ai-issue-service app/main.py MigrationHeaderMiddleware.
const MIGRATION_URL_HEADER = 'x-service-migration-url';       // Node lowercases response headers.
const MIGRATION_DEADLINE_HEADER = 'x-service-migration-deadline';

/**
 * Pure helper: extract MigrationInfo from an http response headers object.
 * Returns null if the headers are absent or the URL is empty.
 *
 * @param {Record<string, string|string[]|undefined>} headers
 * @returns {import('./types').MigrationInfo|null}
 */
function parseMigrationHeaders(headers) {
  if (!headers) return null;
  const rawUrl = headers[MIGRATION_URL_HEADER];
  const url = Array.isArray(rawUrl) ? rawUrl[0] : rawUrl;
  if (!url || typeof url !== 'string' || url.trim() === '') return null;
  const rawDeadline = headers[MIGRATION_DEADLINE_HEADER];
  const deadline = Array.isArray(rawDeadline) ? rawDeadline[0] : rawDeadline;
  /** @type {import('./types').MigrationInfo} */
  const info = { newUrl: url.trim() };
  if (typeof deadline === 'string' && deadline.trim() !== '') {
    info.deadline = deadline.trim();
  }
  return info;
}
  ...
  return new Promise((resolve, reject) => {
    const req = client.request(url, { method, headers, timeout: 90000 }, (res) => {
      ...
      res.on('end', () => {
        ...
        // Best-effort migration banner (throttled, errors swallowed).
        try {
          const migration = parseMigrationHeaders(res.headers);
          if (migration) {
            // Lazy require to avoid module load cycles.
            require('./migration-notifier').maybeShowMigrationBanner(migration);
          }
        } catch { /* notifier must never break a request */ }
...
function serviceRequestStream(method, path, queryParams = {}, onLine = () => {}) {
  ...
  return new Promise((resolve, reject) => {
    const req = client.request(url, { method, headers, timeout: 600000 }, (res) => {
      // Best-effort migration banner — runs once per response, before body streaming.
      try {
        const migration = parseMigrationHeaders(res.headers);
        if (migration) {
          require('./migration-notifier').maybeShowMigrationBanner(migration);
...
function executeProbe(url, headers, timeoutMs, shapeCheck) {
  ...
  return new Promise((resolve) => {
    ...
    req.on('error', (/** @type {any} */ err) => finalize({
      ok: false,
      errorCode: err && err.code ? err.code : 'REQUEST_ERROR',
      error: err && err.message ? err.message : String(err),
      latencyMs: Date.now() - start,
...
module.exports = {
  ...
  MIGRATION_URL_HEADER,
  MIGRATION_DEADLINE_HEADER,
};
