...
/** Timeout for cloning the installer repo (ms) */
const CLONE_TIMEOUT_MS = 120000;
/** Timeout for running the installer script (ms) */
const INSTALLER_TIMEOUT_MS = 60000;
...
function ensureReviewToolInstalled(repoPath, installerPath, repoUrl) {
  ...
  try {
    const execSpec = script.endsWith('.ps1')
      ? { cmd: 'powershell', args: ['-ExecutionPolicy', 'Bypass', '-File', script, repoPath] }
      : { cmd: 'bash', args: [script, repoPath] };
    debug(`Running installer: ${execSpec.cmd} ${execSpec.args.join(' ')}`);
    execFileSync(execSpec.cmd, execSpec.args, {
      cwd: installerPath,
      stdio: ['ignore', 'pipe', 'pipe'],
      encoding: 'utf8',
...
async function runPostPhase2AutoReview(issueNumber, config, options, prePhase2Head) {
  ...
  // Defensive guard: without a pre-image of HEAD we cannot construct a valid
  // review range (an empty `prePhase2Head` would yield `..HEAD`, which `git
  // log` / `git diff` reject). Fall back to skipping rather than running an
  // invalid review prompt.
  if (!prePhase2Head) {
    warning('Skip auto review: pre-Phase 2 HEAD unavailable; cannot scope review range.');
    return;
  }
  ...
  try {
    ...
    if (taskResult && taskResult.skipped) {
      info(taskResult.reason || 'Auto review skipped.');
      if (stashed) {
        try {
