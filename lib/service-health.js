...
async function checkServiceConnectivity(config, opts = {}) {
  ...
  // Promote migration info from whichever probe response carried it.
  // Either probe may surface the header (server adds it on every response).
  const migration =
    (reachability && reachability.migration) ||
    (auth && auth.migration) ||
    null;
