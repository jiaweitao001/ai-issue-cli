...
/** Maximum time to wait for the critique report (ms) */
const CRITIQUE_REPORT_WAIT_MS = 60000;
...
async function runPostPhase2RubberDuck(issueNumber, config, prePhase2Head) {
  ...
  try {
    ...
    await runCopilot(critiquePrompt, config, {
      silent: false,
  ...
  // ── 4. Parse findings ───────────────────────────────────────────────────
  const reportExists = await waitForFile(reportFile, CRITIQUE_REPORT_WAIT_MS);
  if (!reportExists) {
  ...
  try {
    ...
    await runCopilot(fixPrompt, config, {
      silent: false,
      debugMode: process.env.AI_ISSUE_DEBUG === 'true',
      phase: 'phase2'
    });
