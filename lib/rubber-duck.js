 ...
 *
 * Failure modes are non-blocking: any thrown error from AgentRunner, git, or the
...
async function runPostPhase2RubberDuck(issueNumber, config, prePhase2Head) {
  ...
  try {
    ...
    critiqueTaskResult = await runTask(config, {
      ...
      model: resolveAgentCommandSelection(config, 'rubber_duck_critique').model,
      mcpProfile: 'phase2',
  ...
  try {
    ...
    await runTask(config, {
      ...
      repoPath: config.repoPath,
      reportPath: config.reportPath,
      model: resolveAgentCommandSelection(config, 'rubber_duck_fix').model,
      mcpProfile: 'phase2',
 ...
 * AgentRunner returning and the final commit would leak partial fix edits into
