...
function acquireUpdateLock(opts = {}) {
  ...
  if (fd === null) {
    ...
    if (fd === null) {
      const err = new Error(`Failed to acquire update.lock at ${updateLockPath} after clearing stale entry.`);
      err.code = 'LOCK_DIR';
      throw err;
    }
  }
  ...
  try {
    ...
    const err = new Error(`Failed to inspect watch-active.lock: ${e.message}`);
    err.code = 'LOCK_DIR';
    throw err;
  }
