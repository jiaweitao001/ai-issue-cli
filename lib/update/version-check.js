...
function probeRemote(url) {
  ...
  try {
    ...
    const err = new Error(`git ls-remote --tags ${url} failed: ${e.message}`);
    err.code = 'NETWORK';
    throw err;
  }
...
async function checkCliVersion(detected, effectiveChannel, opts = {}, deps = {}) {
  ...
  if (effectiveChannel === 'tag') {
    ...
    const ref = opts.ref;
    if (!ref) {
      const err = new Error("'pinned' channel requires opts.ref");
      err.code = 'NO_REF';
      throw err;
    }
    toRef = ref;
    ...
    const err = new Error(`Unsupported effectiveChannel: ${effectiveChannel}`);
