...
function probeKnowledgeBase(config) {
  if (!config.knowledgeBasePath) {
    return { name: 'Local Knowledge Base', status: true, detail: 'not configured' };
  }
  const kbDir = expandHome(config.knowledgeBasePath);
  const displayPath = displayPathWithHome(kbDir);
  if (!fs.existsSync(kbDir)) {
    return { name: 'Local Knowledge Base', status: false, detail: `${displayPath} missing`, help: 'Run: ai-issue kb download' };
  }
  try {
    ...
    return { name: 'Local Knowledge Base', status: true, detail: `${displayPath} @ ${version} (${count} entries)` };
  } catch (_err) {
    return { name: 'Local Knowledge Base', status: false, detail: `${displayPath} corrupted`, help: 'Run: ai-issue kb verify' };
  }
}
