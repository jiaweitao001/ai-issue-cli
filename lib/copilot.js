 ...
 */
function resolveConfigPaths(configPath) {
      ...
      if (server.env && typeof server.env === 'object') {
        for (const [key, val] of Object.entries(server.env)) {
          if (typeof val === 'string') {
            server.env[key] = val.replace(/\$\{([^}]+)\}/g, (_, varName) => process.env[varName] || '');
          }
        }
      }
    }
 ...
 * @param {string[]} [options.additionalArgs] - Additional command line arguments
...
function runCopilot(prompt, config, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Export service config to process.env so MCP config ${VAR} expansion picks them up
    if (config.serviceUrl && !process.env.AI_ISSUE_SERVICE_URL) {
      process.env.AI_ISSUE_SERVICE_URL = config.serviceUrl;
    }
    if (config.serviceApiKey && !process.env.AI_ISSUE_SERVICE_API_KEY) {
      process.env.AI_ISSUE_SERVICE_API_KEY = config.serviceApiKey;
    }
    ...
    if (mcpConfigPath) {
      const mcpConfigJson = resolveConfigPaths(mcpConfigPath);
    ...
    // copilot is now a standalone .exe, no need for shell
    const spawnOptions = {
