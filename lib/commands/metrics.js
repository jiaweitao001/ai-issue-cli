...
async function cmdMetrics(options = {}) {
  ...
  try {
    ...
    displayMetrics(data, { ...options, ui: shouldUseTableUi(config, options, ui) ? ui : undefined });
  } catch (err) {
 ...
 */
function displayEngineerTable(ui, engineers) {
  ui.table && ui.table((engineers || []).map((eng) => [
    eng.owner || '-',
    eng.total_issues,
    eng.solved,
    eng.failed,
...
function displayMetrics(data, options) {
  ...
  if (by_engineer && by_engineer.length > 0) {
    if (options && options.ui && options.ui.mode === 'tui') {
      displayEngineerTable(options.ui, by_engineer);
