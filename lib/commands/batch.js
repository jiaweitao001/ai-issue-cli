...
async function cmdBatch(issues, options) {
  ...
  async function processIssue(issue) {
    ...
    const issueIndex = results.completed + 1;
    ...
    try {
      log('');
      log(`[${issueIndex}/${issues.length}] 🚀 Starting Issue #${issue}`, colors.bright);
      ...
      const analysisFile = path.join(config.reportPath, `issue-${issue}-analysis.md`);
      fs.appendFileSync(batchLogFile, `\nGenerated Files:\n`);
  ...
  if (results.failed.length > 0) {
    ...
    results.failed.forEach(item => {
      log(`   - #${item.issue}: ${item.error}`, colors.red);
    });
    log('');
