 ...
 */
function createBatchUi(config, options) {
 ...
 * @param {object} event
 */
function emitBatchEvent(ui, event) {
...
async function cmdBatch(issues, options) {
  ...
  const ui = createBatchUi(config, options);
  ...
  async function processIssue(issue) {
    ...
    try {
      ...
      await cmdSolve(
        issue,
        { ...options, skipHeader: true, quiet: true, silent: true },
