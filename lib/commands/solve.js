...
function parseIssueType(content) {
  // More robust matching for issue type
  const guidanceMatch = content.match(/\*\*类型\*\*:\s*(?:📖\s*)?GUIDANCE/i) ||
...
async function cmdSolve(issueNumber, options) {
  ...
  try {
    ...
    let phase1PromptFile = path.join(__dirname, '..', '..', 'PHASE1_RESEARCH_PROMPT_EN.md');
    if (!fs.existsSync(phase1PromptFile)) {
      phase1PromptFile = path.join(__dirname, '..', 'PHASE1_RESEARCH_PROMPT_EN.md');
    }
