 ...
 * @returns {SolveUi}
 */
function createSolveUi(config, options) {
...
async function cmdSolve(issueNumber, options) {
  ...
  const ui = createSolveUi(config, options || {});
  ...
  try {
    ...
    await runPhase2Solution(
      issueNumber, issueUrl, analysisFile, researchContent, issueType, config, options, ui
    );
