...
const { waitForFile, parseBoolean, parseIssueType, enableDebugIfRequested, getRepoFromConfig } = require('../utils');
...
/** Maximum time to wait for a generated report file (ms) */
 ...
 * @returns {{ researchContent: string, issueType: string }}
 */
...
`;
  ...
  await runCopilot(researchPrompt, config, { silent: options.silent, debugMode: options.debug, phase: 'phase1' });

  const researchExists = await waitForFile(researchFile, REPORT_WAIT_TIMEOUT_MS, (elapsed) => {
    if (!options.silent) {
...
`;
  ...
  await runCopilot(solutionPrompt, config, { silent: options.silent, debugMode: options.debug, phase: 'phase2' });

  const analysisExists = await waitForFile(analysisFile, REPORT_WAIT_TIMEOUT_MS, (elapsed) => {
    if (!options.silent) {
