...
function renderLine(line, dryRun) {
  switch (line.type) {
    ...
    case 'issue': {
      ...
      const resource = line.resource_name ? chalk.yellow(` [${line.resource_name}]`) : '';
      const rec = line.recommendation ? ` ${line.recommendation}` : '';
      log(`  #${String(line.number).padEnd(6)} → ${ss.emoji} ${chalk[ss.color](line.status.padEnd(8))} ${chalk.dim(`(${line.reason}${pr})`)}${resource}${rec}`);
      break;
    }
