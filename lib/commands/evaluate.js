...
async function cmdEvaluate(issueNumber, options) {
  ...
  if (!fs.existsSync(analysisFile)) {
    error(`Analysis and solution report does not exist: ${analysisFile}`);
    error('Please run first: ai-issue solve ' + issueNumber);
    process.exit(1);
  }
  
  // Read evaluation prompt (with fallback for different directory structures)
  let evalPromptFile = path.join(__dirname, '..', '..', 'MANUAL_EVALUATION_PROMPT_EN.md');
  if (!fs.existsSync(evalPromptFile)) {
    evalPromptFile = path.join(__dirname, '..', 'MANUAL_EVALUATION_PROMPT_EN.md');
  }
  if (!fs.existsSync(evalPromptFile)) {
    error(`Evaluation prompt file not found`);
    process.exit(1);
  }
