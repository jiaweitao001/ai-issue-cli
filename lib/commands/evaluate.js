...
async function cmdEvaluate(issueNumber, options) {
  ...
  try { validateTaskModel(config, 'evaluation'); } catch (_) {}
  validateConfiguredAgents(config, { taskType: 'evaluation', includeRubberDuck: false });
...
`;
  ...
  try {
    ...
    const taskResult = await runTask(config, {
      ...
      model: resolveAgentCommandSelection(config, 'evaluation').model,
      mcpProfile: 'evaluate',
    ...
    if (taskResult && Array.isArray(taskResult.warnings)) {
      taskResult.warnings.forEach(message => warning(message));
    }
