...
async function cmdUpdate(options = {}) {
  ...
  // 6) P0 SCOPE (shipped): read-only `--check` path end-to-end (mode detection,
  // P1 SCOPE (shipped): link-mode self-update.
  // P2 SCOPE (this commit): copy-mode self-update via managed clone at
  // ~/.ai-issue/source/ai-issue-cli + npm install -g.
  // (No early mode-based reject anymore — both modes flow through the same plan.)
  ...
  // 10) Link-mode-specific preconditions (do not apply to copy mode — the
  //     managed clone is owned by the worker, not the user)
  if (plan.mode === 'link') {
