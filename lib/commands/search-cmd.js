...
async function cmdSearch(query, options = {}) {
  ...
  try {
    ...
    displaySearchResults(query, data, { ...options, ui: shouldUseTableUi(config, options, ui) ? ui : undefined });
  } catch (err) {
 ...
 * @param {Array} results
 */
function displaySearchTable(ui, results) {
  ui.table && ui.table((results || []).map((result) => [
    `#${result.issue}`,
    result.status || '-',
 ...
 * @param {object} [options]
 */
function displaySearchResults(query, data, options) {
  ...
  if (options && options.ui && options.ui.mode === 'tui') {
    displaySearchTable(options.ui, results);
    return;
  }
