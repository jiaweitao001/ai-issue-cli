// @ts-check
/**
 * Watch command — daemon that polls pipeline for queued issues and auto-solves them.
 */

const { loadConfig } = require('../config');
const { log, error, info, success, warning, chalk, debug } = require('../logger');
const { serviceRequest, getServiceUrl } = require('../service-client');
const { enableDebugIfRequested } = require('../utils');
const { applyAgentCommandOptions, validateTaskModel } = require('../agents/command-options');
const { validateConfiguredAgents } = require('../agents/installation-validator');
const { createUi } = require('../ui');
const {
  isUpdateLockActive,
  writeWatchActiveLock,
  unlinkWatchActiveLock,
} = require('../update/lock');

/**
 * Poll the pipeline for queued issues assigned to the given owner.
 * @param {string} repo
 * @param {string} owner
 * @returns {Promise<Array>}
 */
async function fetchQueuedIssues(repo, owner) {
  const { status, data } = await serviceRequest('GET', '/pipeline', null, {
    repo,
    owner,
    status: 'queued',
    limit: 10,
  });

  if (status !== 200) {
    throw new Error(`Pipeline query failed (HTTP ${status})`);
  }

  return Array.isArray(data) ? data : [];
}

/**
 * @param {object} config
 * @param {object} options
 */
function createWatchUi(config, options) {
  if (options && options.ui) return options.ui;
  return createUi({
    flagPlain: !!(options && options.plain),
    flagTui: !!(options && options.tui),
    debug: !!(options && options.debug),
    config,
    env: (options && options.env) || process.env,
    stdout: (options && options.stdout) || process.stdout,
    stdin: (options && options.stdin) || process.stdin
  });
}

/**
 * @param {{ emit?: (event: object) => void }} ui
 * @param {object} event
 */
function emitWatchEvent(ui, event) {
  if (ui && typeof ui.emit === 'function') ui.emit(event);
}

/**
 * @param {{ emit?: (event: object) => void }} ui
 * @param {string} parentTaskId
 * @param {string} issue
 * @param {string} agent
 */
function createIssueUi(ui, parentTaskId, issue, agent) {
  return {
    emit(event) {
      if (!ui || typeof ui.emit !== 'function' || !event || typeof event !== 'object') return;
      ui.emit({
        ...event,
        taskId: event.taskId ? `${parentTaskId}:${event.taskId}` : parentTaskId,
        parentTaskId,
        issue,
        agent,
        dashboardTitle: 'Watch Dashboard'
      });
    }
  };
}

/**
 * Solve a single issue from the pipeline.
 * @param {number} issueNumber
 * @param {object} options - Solve options (branch, pushFork, etc.)
 * @param {{ ui?: { emit?: (event: object) => void } }} [deps]
 * @returns {Promise<boolean>} - true if solved successfully
 */
async function solveIssue(issueNumber, options, deps) {
  const { cmdSolve } = require('./solve');

  try {
    await cmdSolve(
      String(issueNumber),
      {
        ...options,
        branch: true,
        force: true,  // Override triage check since we already filtered by queued
        silent: true,
        skipEval: options.skipEval === true,
      },
      deps || {}
    );
    return true;
  } catch (err) {
    warning(`Failed to solve issue #${issueNumber}: ${err.message}`);
    return false;
  }
}

/**
 * Execute one watch cycle: fetch queued issues → solve each.
 * @param {object} config
 * @param {string} owner
 * @param {object} options
 * @returns {Promise<{processed: number, solved: number, failed: number}>}
 */
async function watchCycle(config, owner, options) {
  const repo = config.repo || process.env.AI_ISSUE_REPO || '';

  const issues = await fetchQueuedIssues(repo, owner);

  if (issues.length === 0) {
    debug('No queued issues found');
    return { processed: 0, solved: 0, failed: 0 };
  }

  info(`Found ${issues.length} queued issue(s) for ${owner}`);

  let solved = 0;
  let failed = 0;
  const ui = options && options.ui;
  const agent = config.agent || 'copilot';

  for (const entry of issues) {
    const issueNumber = entry.issue;
    const entryRepo = entry.repo || repo;
    const issue = String(issueNumber);
    const parentTaskId = `issue-${issue}`;
    const startedAt = Date.now();
    info(`Processing issue #${issueNumber}: ${entry.title || ''}`);
    emitWatchEvent(ui, {
      type: 'task:start',
      taskId: parentTaskId,
      parentTaskId,
      issue,
      label: `Issue #${issue}`,
      startedAt,
      agent,
      dashboardTitle: 'Watch Dashboard',
      plain: false
    });

    // Report solving status to service
    try {
      await serviceRequest('POST', `/pipeline/${entryRepo}/${issueNumber}/solving`);
    } catch (_) {
      debug(`Failed to report solving status for #${issueNumber}`);
    }

    const ok = await solveIssue(issueNumber, options, {
      ui: createIssueUi(ui, parentTaskId, issue, agent)
    });
    const endedAt = Date.now();
    if (ok) {
      solved++;
      success(`Issue #${issueNumber} solved`);
      emitWatchEvent(ui, {
        type: 'task:finish',
        taskId: parentTaskId,
        parentTaskId,
        issue,
        label: `Issue #${issue}`,
        startedAt,
        endedAt,
        durationMs: endedAt - startedAt,
        success: true,
        agent,
        dashboardTitle: 'Watch Dashboard',
        plain: false
      });
    } else {
      failed++;
      emitWatchEvent(ui, {
        type: 'task:finish',
        taskId: parentTaskId,
        parentTaskId,
        issue,
        label: `Issue #${issue}`,
        startedAt,
        endedAt,
        durationMs: endedAt - startedAt,
        success: false,
        agent,
        dashboardTitle: 'Watch Dashboard',
        plain: false
      });
      // Ensure pipeline status is updated to failed so the issue doesn't stay stuck in 'solving'
      try {
        await serviceRequest('POST', `/pipeline/${entryRepo}/${issueNumber}/failed`, null, {
          error: `Watch daemon: solve failed for #${issueNumber}`,
        });
        debug(`Pipeline status updated to failed for #${issueNumber}`);
      } catch (_) {
        debug(`Failed to report failed status for #${issueNumber}`);
      }
    }
  }

  return { processed: issues.length, solved, failed };
}

/**
 * Sleep helper.
 * @param {number} ms
 * @returns {Promise<void>}
 */
function sleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

/**
 * Run one watch iteration: skip if update.lock is active, otherwise
 * write watch-active.lock, run a cycle, unlink watch-active.lock.
 * Extracted from cmdWatch's loop so it's directly testable without timers.
 *
 * @param {object} config
 * @param {string} owner
 * @param {object} options
 * @returns {Promise<{skipped: boolean, result?: object, error?: Error}>}
 */
async function watchIteration(config, owner, options) {
  if (isUpdateLockActive()) {
    debug('ai-issue update is in progress; skipping this cycle');
    return { skipped: true };
  }

  let wroteWatchLock = false;
  try {
    writeWatchActiveLock({ startedAt: new Date().toISOString() });
    wroteWatchLock = true;

    const result = await watchCycle(config, owner, options);
    if (result.processed > 0) {
      info(`Cycle complete: ${result.solved} solved, ${result.failed} failed`);
    }
    return { skipped: false, result };
  } catch (err) {
    warning(`Watch cycle error: ${err.message}`);
    debug(err.stack || '');
    return { skipped: false, error: err };
  } finally {
    if (wroteWatchLock) {
      try { unlinkWatchActiveLock(); } catch (_e) { /* best-effort */ }
    }
  }
}

/**
 * Execute the watch daemon.
 * @param {object} options - { owner, interval, pushFork, debug, ... }
 */
async function cmdWatch(options = {}) {
  const config = loadConfig();
  applyAgentCommandOptions(config, options);
  const owner = options.owner;
  const intervalSec = options.interval || 300;

  if (!owner) {
    error('--owner is required for watch mode');
    return;
  }

  if (!getServiceUrl()) {
    error('Service URL not configured. Set serviceUrl in config or AI_ISSUE_SERVICE_URL env var.');
    return;
  }

  try { validateTaskModel(config, 'research'); } catch (_) {}
  validateConfiguredAgents(config, { taskType: 'research' });

  enableDebugIfRequested(options);
  const ui = createWatchUi(config, options);
  const watchOptions = { ...options, ui };

  log('');
  log(chalk.bold.cyan('👁️  AI Issue Watch Daemon'));
  log(chalk.cyan('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
  log('');
  info(`Owner: ${owner}`);
  info(`Poll interval: ${intervalSec}s`);
  info(`Push fork: ${options.pushFork ? 'yes' : 'no'}`);
  log('');

  // Run in a loop
  let running = true;

  // Handle graceful shutdown
  const shutdown = () => {
    info('Shutting down watch daemon...');
    running = false;
    try { unlinkWatchActiveLock(); } catch (_e) { /* best-effort */ }
  };
  process.on('SIGINT', shutdown);
  process.on('SIGTERM', shutdown);

  while (running) {
    await watchIteration(config, owner, watchOptions);
    if (!running) break;

    debug(`Sleeping ${intervalSec}s until next poll...`);
    await sleep(intervalSec * 1000);
  }

  info('Watch daemon stopped');
}

module.exports = {
  cmdWatch,
  watchCycle,
  watchIteration,
  fetchQueuedIssues,
  sleep,
  _internal: {
    createWatchUi,
    createIssueUi
  }
};
