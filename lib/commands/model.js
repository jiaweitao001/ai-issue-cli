...
async function _cmdSelect(options = {}) {
  ...
  // Build choice items (name = stable identifier returned by ui.prompts.select).
  const choices = items.map((m) => {
    ...
    return {
      name: m.id,
      label: `${m.id.padEnd(28)} ${meta}${tags}${markerSuffix}`
    };
  });
  choices.push({ name: CUSTOM_OPTION_NAME, label: CUSTOM_OPTION_LABEL });
  ...
  const selectedName = await uiPrompts.select(choices, { initialIndex, message: header });
  ...
  let chosenId;
  if (selectedName === CUSTOM_OPTION_NAME) {
    const answer = await uiPrompts.input({ message: 'Enter custom model ID:' });
