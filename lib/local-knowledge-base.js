...
function createSearchIndex(entries) {
  const index = new (/** @type {any} */ (MiniSearch))({
...
class LocalKnowledgeBase {
  ...
  async load() {
    ...
    this._loadPromise = (async () => {
      try {
        ...
        // Reset partial state so a retry after a transient failure can succeed.
        this.manifest = null;
        this.entries = null;
        this.df = null;
        this.searchIndex = null;
  ...
  async checkSha256() {
    const expected = typeof this.manifest.kbSha256 === 'string' ? this.manifest.kbSha256 : '';
    const kbPath = path.join(this.kbDir, 'kb.jsonl');
  ...
  async findSimilarIssues(query, opts = {}) {
    ...
    candidates = candidates.filter(entry => !resourceType || !entry.resource_type || entry.resource_type === resourceType);
    candidates = candidates.filter(entry => !service || !entry.service || entry.service === service);
  ...
  async getResourceInfo(resourceType) {
    ...
    // Mirror ai-issue-service/app/triage.py:75-122: strip "azurerm_" prefix,
    // then try exact match first, then glob match for patterns containing "*".
    // Required because the service emits resource_pattern in bare-glob form
    // (e.g., "key_vault*", "cognitive_*") while CLI callers pass full
    // "azurerm_X" resource names.
    const shortName = resourceType.startsWith('azurerm_')
      ? resourceType.slice('azurerm_'.length)
      : resourceType;
    const candidates = this.entries.filter(entry => entry.type === 'resource_index');
    const exact = candidates.find(entry => entry.resource_type === shortName);
    if (exact) return exact;
    for (const entry of candidates) {
      const pattern = entry.resource_type;
      if (!pattern || !pattern.includes('*')) continue;
      const escaped = pattern.replace(/[.+?^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*');
      if (new RegExp(`^${escaped}$`).test(shortName)) return entry;
    }
    return null;
  }
}
